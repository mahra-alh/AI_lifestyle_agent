...
class ChatResponse(BaseModel):
    ...
    # Set when the agent just prepared a booking that awaits user confirmation.
    # The frontend renders a confirmation card from this.
...
def _dependency_snapshot() -> dict[str, Any]:
    try:
        from ml.recommendation_service import get_model
        _, model_version = get_model()
    ...
    booking_ready = ready and calendar_service["booking_ready"]
    if calendar_service["booking_required"]:
        ready = ready and calendar_service["booking_ready"]
...
async def chat(request: ChatRequest) -> ChatResponse:
    ...
    # If the agent just prepared a booking, attach it so the UI can show
    # the confirmation card. Never let this lookup break the chat response.
    try:
        pending_booking = get_latest_pending_booking_for_user(user_id)
    except Exception:
...
@app.post("/api/bookings/{pending_id}/confirm")
def confirm_booking(pending_id: str) -> dict[str, Any]:
    """Execute a pending booking — only ever triggered by a user click in the UI."""
    result = confirm_pending_booking(pending_id.strip())
    if result.get("status") == "error":
        raise HTTPException(status_code=400, detail=result.get("message"))
    return result


@app.post("/api/bookings/{pending_id}/cancel")
def cancel_booking(pending_id: str) -> dict[str, Any]:
    """Cancel a pending booking without writing anything to the calendar."""
    result = cancel_pending_booking(pending_id.strip())
    if result.get("status") == "error":
        raise HTTPException(status_code=400, detail=result.get("message"))
    return result