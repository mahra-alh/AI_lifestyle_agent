...
app.post("/predict", async (req, res) => {
  ...
  if (input.user_id && input.user_message) {
    const pythonResponse = await fetchJson(`${PYTHON_AGENT_URL}/api/chat`, {
      method: "POST",
      body: JSON.stringify({
    ...
    return res.status(pythonResponse.status).json(pythonResponse.body);
  }
  ...
  res.json({ recommendation, note: "Legacy fallback route. Prefer /api/chat." });
});
